...
uint32_t ret32, low32;
/// Возвращает большую часть от 128: 64x64
FORCE_INLINE uint32_t hmul32(const uint32_t& a,const uint32_t& b){
    asm ("mull %3" : "=a"(low32), "=d"(ret32) : "a"(a), "g"(b) : "cc");
    return ret32;
}
...
FORCE_INLINE uint32_t montmul32(const uint32_t& a,const uint32_t& b,const uint32_t& mod,const uint32_t& inv){
    //a *= b;
    uint32_t high = hmul32(a,b);

    return submod(high, hmul32((low32 * inv), mod), mod);
}


/// 1 * b
FORCE_INLINE uint32_t montmul_1_by_some_32(const uint32_t& b, const uint32_t& mod, const uint32_t& inv){
    //a *= b;

    /// low32 = b
    return mod -  hmul32((b * inv), mod);
}




bool miller_rabin32(const uint32_t& n, uint32_t a) {

    uint32_t inv = n;
    for(int i = 0;i < 4; ++i) inv *= 2 - n * inv;


    uint64_t r = ((uint64_t(1) << 63) % n) << 1;
    if (r > n) r-= n;
    uint32_t r2 = r;

    uint32_t one = montmul_1_by_some_32(r2,n,inv);
    a = montmul32(a,r2,n,inv);
    uint32_t d = n-1;
    uint32_t minus_one = montmul32(d,r2,n,inv);
    ...
    while (d) {
        if (d & 1) {
            x = montmul32(x,a,n,inv);
        }
        a = montmul32(a,a,n,inv);
        d >>= 1;
    }
...
bool is_prime_WTF(uint32_t x) {
    ...
    //_5c(53,59,61,67,71)
    //_5c(73,79,83,89,97)
    if (x<13*13) return (x>1);
    uint64_t h = x;
    h = ((h >> 16) ^ h) * 0x45d9f3b;
    h = ((h >> 16) ^ h) * 0x45d9f3b;
    h = ((h >> 16) ^ h) & 255;
    return miller_rabin32(x,bases[h]);
}
