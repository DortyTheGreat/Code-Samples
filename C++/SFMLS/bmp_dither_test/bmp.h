...
struct RGBAColor{
    ...
    bool operator< (const RGBAColor& color) const{
        if (R != color.R) return R < color.R;
        if (B != color.B) return B < color.B;
        if (G != color.G) return G < color.G;
        return A < color.A;
    }
...
struct RGBAColorRef{
    ...
    void operator= (const RGBAColorRef& color){
        R = color.R;
        G = color.G;
        B = color.B;
        A = color.A;
...
struct BMP {
    ...
    inline RGBAColorRef operator[] (const custom_size_t& index){
        return {data[index],data[index+1],data[index+2],data[index+3]};
    }

    inline RGBAColorRef operator[] (const xy_pair& p){
        int index = get_channels() * (p.y * bmp_info_header.width + p.x);
        return {data[index],data[index+1],data[index+2],data[index+3]};
    }

    inline bool inBounds (const xy_pair& p){
        return (p.x >= 0 && p.y >= 0 && p.y < bmp_info_header.height && p.x < bmp_info_header.width);
    }
